...
PICO 4 的 LSPosed 模块：把一个 Sleep Mode 开关加入 Dock 右侧时间/电源入口打开的二级快捷设置面板。

### 起因

在北京时间 CST (UTC+8)：`2026-08-15 04:20`，协调世界时 UTC：`2026-08-14 20:20`，
太平洋夏令时 PDT (UTC-7)：`2026-08-14 13:20`，美东夏令时 EDT (UTC-4)：`2026-08-14 16:20`，
中欧夏令时 CEST (UTC+2)：`2026-08-14 22:20`，莫斯科时间 MSK (UTC+3)：`2026-08-14 23:20`，
我准备在 VRChat 里 Sleep Mode。充电宝的充电功率干不过头显的耗电，电量越用越少；怒而写出这个模块 😡。现在写完了，准备测试一下这个 Sleep Mode 到底有没有用。


### 已实现

- 在 PICO 系统二级快捷设置面板加入 Sleep Mode 快捷格，复用系统原生外观、焦点和开启状态边框。
- 使用模块自带图标，不修改系统 APK。
...
- 后续计划：可配置睡眠预设、兼容性诊断、长时间 VRChat Sleep Mode 的电量/温度/稳定性实测，以及固件兼容性矩阵。

### 安装与验证

1. 安装 Release 提供的 `sleep-mode.apk`；自行构建时 APK 位于 `mod_vsleep/build/sleep-mode.apk`。
2. 在 LSPosed / Zygisk-Vector 启用模块，作用域只选择 `com.picovr.settings`。
3. 强制停止“设置”或重启头显。
4. 点击 Dock 右侧时间/电源区域，Sleep Mode 位于二级面板底部快捷栏第一格。
...
An LSPosed module for PICO 4 that adds a Sleep Mode toggle to the secondary Quick Settings panel opened from the Dock time/power area.

### Why it exists

At CST (UTC+8) `2026-08-15 04:20`, UTC `2026-08-14 20:20`, PDT (UTC-7) `2026-08-14 13:20`, EDT (UTC-4) `2026-08-14 16:20`, CEST (UTC+2) `2026-08-14 22:20`, and MSK (UTC+3) `2026-08-14 23:20`, I was about to use Sleep Mode in VRChat. My power bank could not keep up with the headset's power draw, so the battery kept draining while charging. I got angry and wrote this module 😡. It is now finished, and I am about to test whether Sleep Mode actually helps.

### Implemented

- Native-looking Sleep Mode Quick Settings tile with module-provided icon; no system APK modification.
...
- Planned: configurable presets, compatibility diagnostics, long VRChat Sleep Mode power/temperature/stability tests, and a firmware compatibility matrix.

### Install

1. Install `sleep-mode.apk` from Releases, or build `mod_vsleep/build/sleep-mode.apk` yourself.
...
4. Open the Dock time/power panel. Sleep Mode is the first tile in the lower Quick Settings row.

---

## Русский

Модуль LSPosed для PICO 4: добавляет переключатель Sleep Mode во вторичную панель быстрых настроек, которая открывается через область времени/питания в Dock.

### Зачем он нужен

В CST (UTC+8) `2026-08-15 04:20`, UTC `2026-08-14 20:20`, PDT (UTC-7) `2026-08-14 13:20`, EDT (UTC-4) `2026-08-14 16:20`, CEST (UTC+2) `2026-08-14 22:20` и MSK (UTC+3) `2026-08-14 23:20` я собирался использовать Sleep Mode в VRChat. Пауэрбанк не справлялся с энергопотреблением шлема, поэтому заряд продолжал уменьшаться даже во время зарядки. Я разозлился и написал этот модуль 😡. Теперь он готов, и я собираюсь проверить, действительно ли помогает Sleep Mode.

### Реализовано

- Нативно выглядящая плитка Sleep Mode в быстрых настройках с иконкой модуля, без изменения системного APK.
...
- В планах: настраиваемые пресеты, диагностика совместимости, длительные тесты Sleep Mode в VRChat и таблица совместимости прошивок.

### Установка

1. Установите `sleep-mode.apk` из Releases или соберите `mod_vsleep/build/sleep-mode.apk` самостоятельно.
...
4. Откройте панель времени/питания в Dock. Sleep Mode будет первой плиткой нижнего ряда быстрых настроек.
